...
def generate_launch_description():
    ...
    # Configuration files
    config_go2_mapping = PathJoinSubstitution([p2p_move_base_pkg_share, 'config', 'mapping.yaml'])
    ...
    lio_nav_bridge_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('lio_nav_bridge'), 'launch', 'lio_nav_bridge_sim.launch.py')
        ),
        launch_arguments={
            'use_sim_time': LaunchConfiguration('use_sim_time'),
            'open_bridge_rviz': "false"
        }.items()
    )
